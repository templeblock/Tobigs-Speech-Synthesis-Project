...
def get_path_dict(data_dirs, hparams, config,data_type, n_test=None,rng=np.random.RandomState(123)):
    ...
    for data_dir in data_dirs:  # ['datasets/moon\\data']
        ...
        if not config.skip_path_filter:
            ...
            new_items = [(path, n) for path, n, n_tokens in items if min_n_frame <= n <= max_n_frame and n_tokens >= hparams.min_tokens] # [('/home/jeon/Desktop/Speech_project/Tacotron-Wavenet-Vocoder/data/IU/1_1_1088.npz', 216),...]
            #print('=====len_new_item====') #1177
            #print(len(new_items))
...
def _prepare_inputs(inputs):  # inputs: batch 길이 만큼의 list
    ...
    #print(max_len) # 155
    #print('========stack===========')
    #a=np.stack([_pad_input(x, max_len) for x in inputs])
    #print(a,a.shape) #(2,155), 155에 맞게 나머지는 0으로 채움(padding)
