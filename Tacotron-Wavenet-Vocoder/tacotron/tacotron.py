...
class Tacotron():
    ...
    def initialize(self, inputs, input_lengths, num_speakers, speaker_id,mel_targets=None, linear_targets=None, loss_coeff=None,
        ...
        with tf.variable_scope('inference') as scope:
            ...
            if self.num_speakers > 1:
                if hp.speaker_embedding_size != 1:    # speaker_embedding_size = 16
                    ...
                    # [N, T_in, speaker_embedding_size] 로 초기화
                    #print('====speaker_embed_table=====')
                    #print(speaker_embed_table.shape) #(2,16)
                    speaker_embed = tf.nn.embedding_lookup(speaker_embed_table, speaker_id) #(2,16)
                ...
                if hp.model_type == 'deepvoice':
                    ...
                    else: #각 사이즈만큼 초기화된 speaker_embed를 fully connected
                        deep_dense = lambda x, dim: tf.layers.dense(x, dim, activation=tf.nn.softsign)   # softsign: x / (abs(x) + 1)
                        before_highway = deep_dense( speaker_embed, hp.enc_prenet_sizes[-1]) #(?,128) cf) enc_prenet_sizes:(256,128)
                        encoder_rnn_init_state = deep_dense( speaker_embed, hp.enc_rnn_size * 2) #(?,256) cf)enc_rnn_size : 128
                        attention_rnn_init_state = deep_dense(speaker_embed, hp.attention_state_size) #(?256) cf)attention_state_size : 256
                        decoder_rnn_init_states = [deep_dense(speaker_embed, hp.dec_rnn_size) for _ in range(hp.dec_layer_num)] #cf)dec_rnn_size = 256, dec_layer_num=2
            ...
            # ==> (N, T_in, 128)
            #pre-net(dropout 기법을 적용한 2층의 fully connected layer로서 overfitting을 방지하고 training이 수렴하는걸 도움)
            ...
            linear_outputs = tf.layers.dense(post_outputs, hp.num_freq)    # [N, T_out, F(num_freq=1025)]
    ...
    def add_loss(self):
        ...
        with tf.variable_scope('loss') as scope:
            hp = self._hparams
            mel_loss = tf.abs(self.mel_targets - self.mel_outputs) #L1 loss

            l1 = tf.abs(self.linear_targets - self.linear_outputs) #L1 loss (절댓값)
            expanded_loss_coeff = tf.expand_dims(tf.expand_dims(self.loss_coeff, [-1]), [-1])

            if hp.prioritize_loss: #false
